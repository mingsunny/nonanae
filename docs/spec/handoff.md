...
| 페이지 | 담당자 | 상태 |
|---|---|---|
| 01. 인트로 / 로그인 / 회원가입 | 민선 | 작성중 |
| 02. 계좌 등록 (회원가입 2단계) | 민선 | 작성중 |
| 03. 그룹 목록 (홈 탭) | 민선 | 작성중 |
| 04. 프로필 (탭) | 민선 | 작성중 |
| 05. 새 그룹 만들기 | 민선 | 작성중 |
| 06. 초대코드로 참여 | 민선 | 작성중 |
| 07. 참여하기 (멤버 매칭) | 민선 | 작성중 |
| 08. 그룹 지출 내역 | | 초안 |
| 09. 그룹 정산 | | 초안 |
| 10. 그룹 요약 | | 초안 |
| 11. 지출 추가/수정 폼 | | 초안 |
...
- ~~**02-onboarding 화면 존치 여부 결정 필요**~~ → **해결됨** (2026-09-20): sep19 프로토타입에서 회원가입이 2단계로 분리되어(1단계 이메일/비밀번호, 2단계 이름/은행/계좌) 계좌 등록 화면(`screen-signup-account`)이 다시 별도 화면으로 존재함. [02-onboarding.md](02-onboarding.md)를 폐기/병합하지 않고 "회원가입 2단계"의 정식 문서로 유지하기로 정리하고, 1단계는 [01-login.md](01-login.md)에서 다루도록 나눔.
