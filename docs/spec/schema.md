...
- **기준**: `settlement-prototype-sep09-yunjung.html`
- 08~13 화면이 실제로 쓰는 데이터 기준. 로그인/온보딩/초대 등 다른 화면 스키마는 `PLAN-sep03-yunjung.md`가 원본
...
```
- `id` 은행계좌 등록 시 자동으로 생기는 내부 식별자
- `user_id FK` 계좌 주인의 `user_id`
- `bank_name` 은행계좌 등록 시 유저가 선택한 은행 명
- `account_number` 은행계좌 등록 시 유저가 입력한 계좌번호
  - 유저당 0개 또는 1개만 존재
...
- `pending_name` 초대 유저가 임의로 등록한 유저 이름
  - `user_id`가 NULL인 경우(대기자인 경우)에만 사용
...
| 버전 | 날짜 | 변경 내용 |
|---|---|---|
| v1.0 | 2026-09-10 | `PLAN-sep09-yunjung.md`(08~11 데이터 스키마 정리)를 이 문서로 이전. 이후 스키마 변경은 이 파일에서 관리 |
