...
카카오 로그인 후 최초 1회 온보딩([02-onboarding.md](02-onboarding.md))에서 생성됨.

| 필드 | 타입 | 설명 |
|---|---|---|
| id | string | 내부 식별자 (`uid('u')`로 생성) |
| kakaoId | string | 카카오 로그인 식별자. `state.users`에서 이 값을 key로 사용 |
| name | string | 이름. 카카오 프로필에서 최초 자동 채움, 온보딩/프로필에서 수정 가능 |
