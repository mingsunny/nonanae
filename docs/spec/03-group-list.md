...
> ⚠️ **게스트 참가자는 이 화면에 진입하지 않음.** 닉네임+PIN 경량 세션으로 참여한 게스트는 세션 자체가 "참여한 그룹 하나"에 scope되어 있어 "내 그룹 목록"이라는 개념이 없음 — 초대코드로 참여하면 곧바로 그 그룹의 내부 화면([08-group-expense-list.md](08-group-expense-list.md))으로 진입하고, 하단 탭바도 홈/프로필 없이 그룹 내부 탭(지출/정산/요약)만 노출됨. 배경: [PLAN-sep03-yunjung.md](../../PLAN-sep03-yunjung.md) "결정된 질문 9" 참고.
