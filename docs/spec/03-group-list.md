...
> ⚠️ **게스트 참가자는 이 화면에 진입하지 않음.** 로그인 없이(이름만으로) 참여한 게스트는 "참여한 그룹 하나"에만 scope되어 있어 "내 그룹 목록"이라는 개념이 없음 — 초대코드로 참여하면 곧바로 그 그룹의 내부 화면([08-group-expense-list.md](08-group-expense-list.md))으로 진입하고, 하단 탭바도 홈/프로필 없이 그룹 내부 탭(지출/정산/요약)만 노출됨. 재입장 시 PIN 등 별도 인증은 없음(2026-09-12 확정). 배경: [PLAN-sep20.md](../../PLAN-sep20.md) "결정된 질문" 7번 참고.
...
- 내가 속한 그룹 목록 표시 (그룹명, 멤버 수, 총 지출 금액). 가장 최근에 만들거나 참여한 그룹이 위 — `Group`에는 만든 시각이 없어서 내 `Member.joinedAt` 내림차순 (2026-09-20)
- 그룹 목록 항목 클릭 시 해당 그룹 내부([08-group-expense-list.md](08-group-expense-list.md))로 이동
- "＋ 새 그룹 만들기" / "초대코드 입력" 진입 버튼
- 그룹이 하나도 없는 최초 사용자에게 그룹 생성을 유도하는 1회성 안내 문구 노출
...
- 로그인 성공 직후, 또는 회원가입 2단계([02-onboarding.md](02-onboarding.md)) 완료 직후 (`afterAuthed()`) 자동 진입, 항상 "홈" 탭이 기본 선택
...
- 첫 그룹 생성 유도 안내 문구(`group-create-hint`, "그룹을 만들고 정산을 시작해요") — 조건 충족 시에만 노출. sep10까지 있던 "새 그룹 만들기" 버튼의 글로우(pulsing) 강조는 sep19에서 제거되어 문구만 남음
...
| "＋ 새 그룹 만들기" 클릭 | 안내 문구를 본 것으로 처리(이후 다시 노출 안 됨) 후 [05-create-group.md](05-create-group.md)로 이동 |
...
- 첫 그룹 생성 안내 문구는 **사용자당 1회만** 노출됨 — 그룹이 0개이면서 아직 안내를 안 본 경우(`!seenGroupCreateCoach`)에만 표시되고, "＋ 새 그룹 만들기"를 한 번이라도 누르면 다시 뜨지 않음 (sep19에서 버튼 강조 효과는 없어지고 문구만 남음)
...
- 2026-09-20: sep19 프로토타입 기준으로 수정. 첫 그룹 생성 안내에서 버튼 글로우 강조가 제거된 점 반영, 진입 경로를 로그인 성공/회원가입 2단계 완료로 정정 (민선)
