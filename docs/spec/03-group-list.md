...
- (신규) 게스트는 이 화면 자체에 진입하지 않으므로 위 예외처리는 모두 정식 회원 기준 — 게스트가 여러 그룹에 참여하고 싶으면 각 그룹의 초대코드를 각각 사용해야 하고(그룹별로 별도 게스트 세션), 그룹들을 한 곳에서 모아보는 기능은 없음. 여러 그룹을 관리하고 싶은 사용자는 정식 회원으로 가입해야 함(게스트 기록은 새 계정으로 옮겨지지 않음)
