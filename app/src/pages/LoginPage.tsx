 ...
 * 인트로를 건너뛰고 로그인 화면으로 온다 (04 액션 & 결과).
 */
export default function LoginPage() {
  const navigate = useNavigate()
  const location = useLocation()
  const startAtLogin = (location.state as { step?: 'login' } | null)?.step === 'login'

  const [step, setStep] = useState<Step>(startAtLogin ? 'login' : 'intro')
  ...
  switch (step) {
    ...
    case 'login':
      return <LoginScreen email={loginEmail} onEmailChange={setLoginEmail} onBack={() => setStep('intro')} />
    case 'signup':
      return (
        <SignupScreen
          draft={draft}
