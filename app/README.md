...
- 앱을 처음 열면 로그아웃 상태라 인트로(01)부터 보인다. 테스트 계정 `a@naver.com` / `aaaaaaaa`(테스트)로 로그인하면 데모 그룹 "제주도 여행"(멤버 4명, 지출 6건, 알림 2건)의 방장이다. 로그인할 수 있는 계정은 이 계정과 새로 가입한 계정뿐이다(데모 유저들은 이름만 있음).
